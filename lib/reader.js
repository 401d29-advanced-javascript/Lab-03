...
const readAll = (paths, callback) => {
  ...
  readOne(paths[0], (err, data) => {
    if (err) {
      ...
      contents.push(data.toString().trim());//converts data in the text file to a string and 
                                            //removes unecessary spaces on the ends with .trim() method
    }
  });

  readOne(paths[1], (err, data) => {//are all asynchronous, but the order isn't set
    if (err) {
      callback(err);
    }
    else {
